...
- `group_map()` is now used for iterating on groups of grouped tibbles, characterised by .x and .y as before.  It however makes no assumptions about the return type of each operation, combining results in a list - similar to `purrr::map()`.
- The previous behaviour was renamed to `group_modify()`, which always returns a grouped tibble, combining returned tibbles by evaluating each operation with a reconstructed grouping structure.<br><br>
...
iris_grouped %>% 
  ...
  # Tidyup to present
  func_tidy_present(., header = "Result of the *Group Modify* method applying the *quantile* function")
```