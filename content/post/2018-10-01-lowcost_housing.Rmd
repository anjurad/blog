...
A *state change* in this context refers to the shift in statuses of multiple activities performed in the delivery of a project, the project forming part of a more extensive body programme of works (concentrated portfolio of project activities).

We could attempt this through Excel, and perhaps we'll be successful as the current dataset only contains c. 60,000 rows and 12 columns, spread roughly equally in 12 spreadsheets.

It requires much cutting and pasting, and then complicated and nested formulas to create features necessary for the required computations.  It is a tall order, and undoubtedly fraught with issues and opportunities to introduce mistakes.  

**Excel does not scale well**<br>
The single biggest issue with using Excel for this type of work is that it does not scale well.  Increasing the column and row count ten-fold will eventually unleash the spinning wheel of death.  If not, then the additional reporting requirements that come with a maturing project will.

This post demonstrates some techniques and approaches using [R](https://www.r-project.org) to transform data and create features to gain insight into *state changes* of *large-scale programmes over time*. 
...
The approaches and techniques described in this post also suit other industries, including but not limited to:
...
# Scenario
In 1994 the [African National Congress](https://en.wikipedia.org/wiki/African_National_Congress) became South Africa's ruling governing political party.  They pledged to address inequalities caused by years of apartheid, specifically to tackle poverty and focus on education and spreading wealth and opportunity.
...
The *fictitious scenario* for this case study is a large-scale South African initiative that provided low-cost housing between 2003 and 2011 in selected [townships](https://en.wikipedia.org/wiki/Township_(South_Africa)), a local term for informal settlements.
...
**[Part 3](#part3)** explores the data to reveal aspects, insights of, and commentary on the content.

**[Part 4](#part4)**.   Exception reporting allows a user to focus on exceptions in the programme.  Progress and Status reporting provides a snapshot of the current state to date, which is especially useful if coupled with targets and performance indicators.  Prognosis offers an estimate of the future state, allowing the users to anticipate and prepare.

**[Part 5](#part5)**.  Evaluating supplier performance is a cornerstone of contractual reporting, especially when terms may result in financial or other penalties and rewards.  Here we consider `duration`, which is a measure of performance particularly suitable for time-critical activities.
...
## Part 4: Exception, Progress & Status and Prognosis Reporting
*Exception* reporting allows users to focus on exceptions in the programme. 
...
Here we consider `duration`, which is a measure of performance particularly suitable for time-critical activities.
