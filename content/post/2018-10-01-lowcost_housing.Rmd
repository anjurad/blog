...
The scenario for this tutorial is a large-scale South African initiative providing low-cost housing in selected [townships](https://en.wikipedia.org/wiki/Township_(South_Africa)), a local term for informal settlements.

The purpose of this tutorial is to demonstrate some techniques to transform data and create features to gain insight into *state changes* of *large-scale programmes over time*. 

A *state change* in this context infers the change to the statuses of multiple activities performed in the delivery of a project, the project forming part of a larger body programme of works (collective portfolio of project activities).
...
The process is initiated by a Statement of Works `SOW` document, committing the delivery of a low-cost house.  This leads to a contract being drawn up with the house manufactured and installed once legal rights are obtained.
...
The structure above reveals 4 major milestones in the delivery of a low-cost house.  We are interested in last 3 Milestones, namely `Contract`, `Manufacturing` and `Installation`, as per the process described in the *[How](#process)* section.
...
These functions will be explained in the following section.
...
The source data is passed through the functions listed above, resulting in the creation of 3 new tables for each report, each a derivative of the project state snapshot.
...
The section below explores each one of the three newly created tables described above.

<a id="milestone_matrix"></a>

#### Milestone Matrix

The `Milestone Matrix` is an automatic extract of each of the milestones contained within the source data.  The `func_add_milestone_matrix` function returns all fields that end with `Forecast` and/ or `Actual`, isolating the `Milestone` name and listing the fields required to provide the `Forecast` and `Actual` dates respectively.  These fields will be used contextually to create Milestone related features.
...
The following code chunk returns the `head` of the of the unnested `ms_long` tibble.

```{r func_add_ms_long}
df_enhanced %>% 
  
  # select the first nested tibble
  head(1) %>% 
...
# return the table head, grouping features for each Milestone
df_enhanced %>%
  head(1) %>%
  select(ms_wide) %>%
  unnest() %>%
...
In this section, we will dive into the detail and reveal aspects of the content. 
...
The slope of the upper-most curve suggests that the `Contract` activity rate of delivery has flattened out during 2010. The other two activities are catching up, largely running in parallel, consuming the pot of *Contract not Installed*.  
...
The rate of delivery in `Contracts` however increased in the 2 years following this event. I suspect that it had achieved critical mass after accruing latent potential during the previous 3 years of activity, subsequently yielding a greater and sustained throughput.  
...
This is achieved by comparing sequential versions of reports, each with detail about underlying jobs and milestones, and extracting and categorising differences between these.
...
The `net` facet show the new volume per Milestone per month.  The large bars at the end of 2011 largely represent activities forecasted beyond 2011.
...
### Evaluating Performance - Duration
...
plot_data %>%
  ...
  # plot the volume by supplier for each supplier at the base of the violin plot
  geom_text(
...
Projects delivered later in the programme are likely to be *more problematic*, a possible explanation why *JG Holdings* took longer to complete activities.  Worth investigating this hypothesis in the future.

*Independents*, on the other hand, are made up of small outfits, making them lean and agile compared with larger suppliers, who in turn are able to scale significantly more.

<hr>

## Summary
The analysis demonstrated in this tutorial is not comprehensive and exhaustive but provides an overview of useful functions to analyse and report state changes of large-scale programmes over time.
