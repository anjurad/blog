...
tmp_exercise_data <- 
  ...
  mutate_at("Date", list(data = map), func_create_data) %>% 
...
files_expanded <-
  ...
  mutate_at("file_name", list(file_source = str_extract), pattern = "\\.(csv|xlsx)$") %>% 
  
  # dynamically import the files and nest the file column
  mutate_at("file_name", list(file = map), func_get_file_dynamic) %>% 
